...
const ProductMap = () => {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const res = await axios.get('http://16.171.149.3:3000/product/getAllProducts');
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://16.171.149.3:3000/product/deleteProduct/${id}`);
  ...
  const handleSave = async (id) => {
    try {
      ...
      await axios.put(`http://16.171.149.3:3000/product/updateProduct/${id}`, updated);
      setEditId(null);
      fetchProducts();
    } catch (error) {
